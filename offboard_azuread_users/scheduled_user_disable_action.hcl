...
pipeline "user_disable_action" {
  ...
  step "pipeline" "ad_user_account_status" {
    ...
    for_each = step.pipeline.list_issues.output.issue.issues != null ? { for each_issue in step.pipeline.list_issues.output.issue.issues : each_issue.id => each_issue if strcontains(each_issue.fields.summary, "Disable") && each_issue.fields.status.name == "Approval Done" } : tomap({})
